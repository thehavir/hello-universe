...
void main() {
  group('Test fetching image', () {
    ...
    test('Test fetch image (APOD) successful', () async {
      final Apod apod = await nasaApodRepository.fetchApod();

      expect(apod.url, isNotNull);
    });

    test('Test fetch image list successful', () async {
      final List<Apod> imageList = await nasaApodRepository.fetchImageList(
    ...
    test('Test fetch image list in boundaries successful', () async {
      ...
      final List<Apod> imageList2 = await nasaApodRepository.fetchImageList(
        startDate: '1995-06-27',
        endDate: '1995-06-16',
      );
      ...
      final List<Apod> imageList3 = await nasaApodRepository.fetchImageList(
        startDate: '1995-06-16',
        endDate: '1995-06-16',
      );

      expect(imageList3.length, 1);
      final List<Apod> imageList4 = await nasaApodRepository.fetchImageList(
    ...
    test('Test fetch Apod of a specific date', () async {
      final Apod apod = await nasaApodRepository.fetchApod(date: '2016-02-06');

      expect(apod.url, isNotNull);
    });

    test('Test thumbnail url is not null for Apod videos', () async {
      final Apod apod = await nasaApodRepository.fetchApod(date: '2016-02-09');
